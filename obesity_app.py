...
if page == "Data":
    ...
    if st.session_state.df is not None:
        ...
        if st.checkbox("Show missing values per column"):
            mv = st.session_state.df.isnull().sum()
            st.write(mv[mv>0])
    ...
    if df is None:
        ...
        if st.button("Apply preprocessing and split data"):
            ...
            if encode_type == "Label" and len(categorical_cols) > 0:
                ...
                st.success("Preprocessing (ColumnTransformer) applied and data split.")
...
# Page: Train & Tune
    ...
    if st.session_state.get('X_train') is None:
        ...
        if st.button("Train"):
            ...
            st.success("Models trained. Proceed to Evaluation page to see results.")
...
# Page: Inference
    ...
    for col in feat_cols:
        ...
        if df[col].dtype in ['int64', 'float64']:
            ...
            if col.lower() == "age":
                sample[col] = st.slider("Age", 1, 100, int(df[col].median()))

            elif col.lower() == "height":
                sample[col] = st.number_input("Height (m)", 0.5, 2.5, float(df[col].median()))

            elif col.lower() == "weight":
                sample[col] = st.number_input("Weight (kg)", 10.0, 300.0, float(df[col].median()))
            ...
            if len(options) <= 5:
                sample[col] = st.radio(col, options)
            else:
                sample[col] = st.selectbox(col, options)
    ...
    if st.button("Predict"):
        ...
        if preproc[0] == 'label':
            ...
            sample_df = pd.DataFrame(
                imputer.fit_transform(sample_df),
